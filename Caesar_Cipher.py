...
class Message(object):
    ...
    def build_shift_dict(self, shift):
        ...
        shift_dict = {}
        for char in lower: 
            location = lower.find(char)
            location += shift 
            if location >= 26: 
                location -= 26
            shift_dict[char] = lower[location]
            
        for char in upper: 
            location = upper.find(char)
            location += shift 
            if location >= 26: 
                location -=26 
            shift_dict[char] = upper[location]
        return shift_dict
...
class CiphertextMessage(Message):
    ...
    def decrypt_message(self):
        ...
        the maximum number of you may choose 
        ...
        completed = False 
        while(True):
            dictionary = Message.build_shift_dict(self, 26 - s)
            newText = Message.apply_shift(self, 26 - s)
...
def decrypt_story():
    story = get_story_string() #encrypted
